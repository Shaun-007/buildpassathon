...
export default function StudentDashboard({ user }) {
  ...
  const handleApply = async (e) => {
    ...
    const now = new Date();
    const leavingTime = new Date(fromDateTime);
    const returningTime = new Date(toDateTime);

    if (leavingTime < now) {
      setMessage({
        text: 'Leaving time cannot be in the past.',
        type: 'error'
      });
      return;
    }

    if (returningTime <= leavingTime) {
      setMessage({
        text: 'Return time must be after leaving time.',
        type: 'error'
      });
      return;
    }
        ...
        <div className="pass-list">
          {passes.map((pass) => (
            <div key={pass.id} className="pass-card">
              ...
              {pass.status === 'APPROVED' && !pass.isBlocked && (
                <button
                  type="button"
              ...
              {pass.isBlocked && <p className="message error">INVALID — BLOCKED BY GA{pass.blockReason ? `: ${pass.blockReason}` : ''}</p>}
            </div>
          ))}
        </div>
      )}
